...
import oled.app.runutils.BKHandling._
import oled.logic.lookaheads.LookAheadUtils
...
class Globals(val entryPath: String) extends LazyLogging {
  ...
  * */
  ...
  val headAtomSignatures: List[AtomSignature] = {
    MODEHS.map(x => new AtomSignature(x.predSymbol, x.arity))
  }
  ...
  */
  def EXAMPLE_COUNT_RULE = {
    ...
    val typePreds = targetPred.getTypePredicates(this.MODEHS ++ this.MODEBS).mkString(",")
