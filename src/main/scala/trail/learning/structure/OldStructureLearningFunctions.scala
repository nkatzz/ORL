...
object OldStructureLearningFunctions extends ASPResultsParser with LazyLogging {
  ...
  def generateNewRulesOLED(topTheory: List[Clause], e: Example, initorterm: String, globals: Globals) = {
  ...
  def KSGeneration(topTheory: List[Clause], examples: Example, inps: RunningOptions, mistakes: Set[String] = Set()) = {
    ...
    val file = trail.app.utils.Utils.dumpToFile(inps.globals.BK)
  ...
  def generateKernel1(examples: Map[String, Set[String]], fromWeakExmpl: Boolean = false,
    ...
    for (x <- abducedAtoms) {
      ...
      for (i <- 0 to Globals.glvalues("variableDepth").toInt) {
        ...
        toASPprogram(program =
          examples("annotation").toList ++
            examples("narrative").toList ++
            program ++
            List(s"\n#include " + "\"" + bkFile + "\".") ++
  ...
  * */
  ...
  def solveASP(task: String, aspFile: String, fromWeakExmpl: Boolean = false): List[AnswerSet] = {
    ...
    val command = Seq("clingo", "--opt-mode=opt --opt-strategy=usc", aspFile, mode, with_atom_undefiend, aspCores)
