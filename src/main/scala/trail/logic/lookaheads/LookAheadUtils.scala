...
package trail.logic.lookaheads

import trail.logic.parsers.ClausalLogicParser
import trail.logic.{Literal, Variable}
...
*
* */
  class LinkingVariable(val variable: Variable, val appearsInLiteral: Int, val positionInLiteral: Int)
  ...
  class LookAheadSpecification(val lookAheadSpecification: String) extends LookAheadsParser {
    ...
    private def isVariableUncommon(v: Variable) = {
      !(litToBeAdded.getVars.contains(v) && litToLinkTo.getVars.contains(v))
    }
    private def variableExists(v: Variable) = {
      litToBeAdded.getVars.contains(v) || litToLinkTo.getVars.contains(v)
    }
    /*
