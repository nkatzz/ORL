...
import trail.logic.parsers.{ClausalLogicParser, ModesParser, PB2LogicParser}
...
object Literal {
  ...
  /* Using the Parboiled2 parser (faster). */
...
case class Literal(
  ...
  def tostringNoNegation: String = terms match {
    case List() => predSymbol
    case _ =>
      val prefix = predSymbol
      prefix + "(" + (for (
        a <- terms; x = a match {
          case x @ (_: Constant | _: Variable | _: Literal) => x
          case _ => throw new RuntimeException(s"Unexpected type of inner term while parsing Literal: $this")
    ...
    remaining match {
      case head :: tail => head match {
        ...
        case (x: Literal, y: ModeAtom) =>
          ...
          val pop = accum.last
          this.variabilize(
            List(Literal(predSymbol = pop.predSymbol, terms = pop.terms ::: varbed, isNAF = pop.isNAF)),
