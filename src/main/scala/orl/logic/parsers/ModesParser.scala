...
package orl.logic.parsers

import com.typesafe.scalalogging.LazyLogging
import orl.logic.{Constant, LogicalExpression, ModeAtom, PlmrkConst, PlmrkNeg, PlmrkPos}
...
final class ModesParser extends JavaTokenParsers with LazyLogging {
  ...
  def modeAtom: Parser[ModeAtom] =
    (naf ~ lowerCaseIdent ~ inner ^^ { case not ~ x ~ y => ModeAtom(predSymbol = x.toString, args = y, isNAF = true) }
      | lowerCaseIdent ~ inner ^^ { case x ~ y => ModeAtom(predSymbol = x.toString, args = y) })
